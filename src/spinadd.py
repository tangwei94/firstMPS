...
def init_newpsi(sigma, As, Bs): # not tested
    ...
    if (matprod_l.shape[0] > matprod_l.shape[1]):
        diff_shape = (matprod_l.shape[0], matprod_l.shape[0] - matprod_l.shape[1])
    ...
    if (matprod_r_dag.shape[0] > matprod_r_dag.shape[1]):
        diff_shape = (matprod_r_dag.shape[0], matprod_r_dag.shape[0] - matprod_r_dag.shape[1])
    ...
    if len(sigma1_rvsd) < dim:
        sigma1_rvsd = np.append(sigma1_rvsd, np.zeros(dim - len(sigma1_rvsd)))
